    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnCreateConversation.setOnClickListener {
            ...
            if (participantsText.isNotEmpty()) {
                val usernames = participantsText.split(",", "(il vous suit)").map { it.trim() }
                    .filter { it.isNotEmpty() }

                lifecycleScope.launch {
                    val conversationsExists =
                        conversationViewModel.checkIfConversationExists(usernames, sessionManager)
                    if (conversationsExists) {
                        Toast.makeText(
                            this@CreateConversationActivity,
                            "La conversation existe deja",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                ...
                Toast.makeText(this, "Veuillez saisir au moins un username", Toast.LENGTH_SHORT)
                    .show()
            }
        }
    }
        ...
        lifecycleScope.launch {
            try {
                ...
                for (user in suggestionsUsernames.take(5)) {
                    val follows =
                        conversationViewModel.isUserFollowingCreator(currentUserId, user.id)
                    if (!follows) {
                ...
                allParticipants.setOnItemClickListener { _, _, position, _ ->
                    ...
                    if (selectedUser != null && !selectedUser.startsWith("Liste") && !selectedUser.startsWith(
                        ...
                        if (!usernames.contains(selectedUser)) {
                            val newText =
                                if (currentText.isEmpty()) selectedUser else "$currentText, $selectedUser"
